...
const MANIFESTO_LIMITED_SNAP_DURATION = 1380;
const MANIFESTO_SNAP_COOLDOWN = 180;
const MANIFESTO_WHEEL_GESTURE_IDLE = 240;
const MANIFESTO_TOUCH_GESTURE_IDLE = 280;
const MANIFESTO_LIMITED_SNAP_COOLDOWN = 900;
const MANIFESTO_LIMITED_WHEEL_GESTURE_IDLE = 980;
const MANIFESTO_LIMITED_TOUCH_GESTURE_IDLE = 760;
...
function shouldLimitManifestoToSingleStep() {
  return manifestoLimitedStepQuery.matches;
}
...
function shouldStopAtHeroStrip(direction) {
  if (!shouldLimitManifestoToSingleStep() || !heroStrip || direction <= 0 || heroStripInputLocked || heroStripSnapFrame || manifestoSnapFrame) {
    return false;
  }
...
function guardManifestoStepLimit(direction) {
  if (!shouldLimitManifestoToSingleStep() || !manifestoSection || reducedMotionQuery.matches || manifestoSnapFrame || heroStripSnapFrame) {
    return false;
  }
  ...
  if (direction > 0 && currentStage > settledStage + 0.62) {
...
function animateManifestoScrollTo(targetY, targetStage) {
  ...
  const startedAt = performance.now();
  const duration = getManifestoSnapDuration();
  const lenis = window.ceylonLenis;
  const metrics = getManifestoMotionMetrics();
...
function handleHeroStripTouchStart(event) {
  if (!shouldLimitManifestoToSingleStep()) return;
...
function handleHeroStripTouchMove(event) {
  if (!shouldLimitManifestoToSingleStep() || heroStripTouchStartY === null || heroStripTouchStartX === null) return;
  const touch = event.touches?.[0];
  if (!touch) return;

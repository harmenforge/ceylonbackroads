...
function initSmoothScroll() {
  ...
  const mobileBlogFeed = body.classList.contains("blog-page") && window.matchMedia("(max-width: 760px)").matches;
  if (mobileBlogFeed) {
    document.documentElement.classList.add("native-mobile-blog-scroll");
    return;
  }

  document.querySelectorAll(".faq-content, .faq-sidebar, .blog-projects-list").forEach((element) => {
    element.setAttribute("data-lenis-prevent", "");
  });

  const mobileScroll = window.matchMedia("(max-width: 940px), (pointer: coarse)").matches;
  const touchProfile = mobileScroll
    ? {
        syncTouchLerp: 0.115,
  ...
  document.documentElement.classList.toggle("touch-scroll-enhanced", mobileScroll);
  ...
  const lenis = new window.Lenis({
    ...
    duration: touchProfile.duration,
    easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
    lerp: touchProfile.lerp,
    syncTouch: true,
    syncTouchLerp: touchProfile.syncTouchLerp,
    touchInertiaExponent: touchProfile.touchInertiaExponent,
    touchMultiplier: touchProfile.touchMultiplier,
    stopInertiaOnNavigate: true,
    prevent: (node) => Boolean(node.closest?.("[data-lenis-prevent], .gallery-lightbox, .preference-menu.is-open, .price-currency-control.is-open, .booking-select-options")),
  });
  ...
  const syncLenisState = () => {
    const shouldStop = body.classList.contains("is-loading") || body.classList.contains("menu-open") || body.classList.contains("gallery-lightbox-open");
    if (shouldStop) lenis.stop();
    else lenis.start();
  };

  syncLenisState();
  new MutationObserver(syncLenisState).observe(body, { attributes: true, attributeFilter: ["class"] });

  window.ceylonLenis = lenis;
}
...
function finishLoader() {
  const minDuration = window.matchMedia("(prefers-reduced-motion: reduce)").matches ? 120 : 1550;
...
if (shouldSkipLoader) {
  ...
  window.setTimeout(() => {
    if (body.classList.contains("is-loading")) finishLoader();
  }, 4200);
}
...
function syncCurrencyMenus(currency, animate = true, userInitiated = false) {
  ...
  currencyMenus.forEach((control) => {
    ...
    const compact = control.hasAttribute("data-compact-currency");
    const label = compact
...
const manifestoHeadingWords = Array.from(document.querySelectorAll(".manifesto-copy h2.scroll-highlight .highlight-word"));
const manifestoBodyWords = Array.from(document.querySelectorAll(".manifesto-copy .manifesto-body .highlight-word"));
const manifestoWords = [...manifestoHeadingWords, ...manifestoBodyWords];
...
const MANIFESTO_SLIDE_START = 0.54;
const MANIFESTO_SLIDE_LENGTH = 0.45;
...
manifestoWords.forEach((word) => {
  word.dataset.highlightText = word.textContent.trim();
});
...
function getEasedManifestoStage(stage, maxStage) {
  ...
  return stageBase + smoothStep(clampedStage - stageBase);
}
...
function animateManifestoScrollTo(targetY, targetStage) {
  ...
  const tick = (now) => {
    const progress = clamp01((now - startedAt) / MANIFESTO_SNAP_DURATION);
    ...
    manifestoSnapCooldownUntil = performance.now() + MANIFESTO_SNAP_COOLDOWN;
    window.scrollTo(0, targetY);
    lenis?.start?.();
...
function snapManifestoSlide(direction) {
  ...
  if (direction > 0) {
    if (metrics.progress < MANIFESTO_COPY_REST_PROGRESS - 0.025) {
      targetStage = -1;
...
function isManifestoSequenceZone(metrics = getManifestoMotionMetrics(), direction = 0) {
  ...
  const approachingFromBelow = false;
...
function releaseManifestoTouchGesture(delay = MANIFESTO_TOUCH_GESTURE_IDLE) {
...
function updateManifestoMotion() {
  ...
  const copyExit = smoothStep((sectionProgress - 0.52) / 0.14);
  const photoStageProgress = stage <= -0.995 ? 0 : 1;

  manifestoCopy?.style.setProperty("--manifest-copy-opacity", "1");
  manifestoCopy?.style.setProperty("--manifest-copy-x", `${lerp(0, -window.innerWidth, copyExit).toFixed(2)}px`);
  manifestoCopy?.style.setProperty("--manifest-copy-y", "0px");
  manifestoPhotoFrame?.style.setProperty("--manifest-photo-opacity", photoStageProgress.toFixed(4));

  const roundedStage = Math.min(maxStage, Math.max(-1, Math.round(currentStage)));
  if (!manifestoSnapFrame && Math.abs(currentStage - roundedStage) < 0.08) {
    manifestoSettledStage = roundedStage;
  }
  ...
  manifestoSlideTitles.forEach((title, index) => {
    ...
    const titleReveal = 1 - smoothStep((titleDistance - 0.02) / 0.24);
...
if (bookingForm) {
  ...
  if (requestedRoute && route) {
    const requestedOption = Array.from(route.options).find((option) => option.dataset.routeSlug === requestedRoute);
    if (requestedOption) {

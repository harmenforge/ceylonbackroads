...
function ensureHeaderPhoneMenu() {
  const nav = header?.querySelector(".nav");
  const bookButton = nav?.querySelector(".header-book-route");
  if (!nav || !bookButton || nav.querySelector(".header-phone-menu")) return;

  const control = document.createElement("div");
  control.className = "header-phone-menu notranslate";
  control.setAttribute("translate", "no");
  control.setAttribute("data-header-phone", "");
  control.innerHTML = `
    <button class="header-phone-trigger" type="button" aria-expanded="false" aria-label="Contact Ceylon Backroads by phone or WhatsApp">
      <svg class="header-phone-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
        <path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.31-.31.76-.4 1.17-.27 1.29.43 2.66.66 4.07.66.64 0 1.16.52 1.16 1.16v3.48c0 .64-.52 1.16-1.16 1.16C10.38 21.87 2.13 13.62 2.13 3.35c0-.64.52-1.16 1.16-1.16h3.49c.64 0 1.16.52 1.16 1.16 0 1.41.22 2.78.65 4.07.13.41.04.86-.27 1.17l-2.2 2.2Z" />
      </svg>
    </button>
    <div class="header-phone-options" aria-hidden="true">
      <a href="https://wa.me/94776650190" target="_blank" rel="noreferrer"><span>WhatsApp message</span></a>
      <a href="tel:+94776650190"><span>Phone call</span></a>
    </div>
  `;
  nav.insertBefore(control, bookButton);

  const trigger = control.querySelector(".header-phone-trigger");
  const options = control.querySelector(".header-phone-options");
  const setOpen = (open) => {
...
function syncMobileHeaderMenuStructure() {
  ...
  const companyGroup = Array.from(navLinksPanel.querySelectorAll(".nav-group")).find((group) => {
    return group.querySelector("strong")?.textContent.trim().toLowerCase() === "company";
  });
  const faqLink = companyGroup ? Array.from(companyGroup.querySelectorAll("a")).find((link) => link.textContent.trim().toLowerCase() === "faq") : null;
  const termsLink = companyGroup?.querySelector("[data-mobile-terms-link]");
  ...
  if (mobileHeaderQuery.matches) {
    if (companyGroup && !termsLink) {
      const link = document.createElement("a");
      link.href = "terms.html";
      link.textContent = "Terms";
      link.setAttribute("data-mobile-terms-link", "");
      if (faqLink) faqLink.insertAdjacentElement("afterend", link);
      else companyGroup.appendChild(link);
    }
...
function updateHeaderBookingMorph(progress) {
  ...
  const headerPhoneMenu = header?.querySelector(".header-phone-menu");
  if (!headerButton && !headerPhoneMenu) return;

  const setHeaderActionFill = (fill) => {
    [headerButton, headerPhoneMenu].filter(Boolean).forEach((element) => {
      element.style.setProperty("--header-book-fill", fill.toFixed(4));
      element.style.setProperty("--header-book-fill-percent", `${(fill * 100).toFixed(1)}%`);
    });
  };
...
function setHeaderState() {
  ...
  const headerLayoutProgress = innerPage ? 1 : smoothStep(progress / 0.24);
  const headerVisualProgress = innerPage ? 1 : progress;
  const effectiveGroupOpacity = groupOpacity;
  const effectiveCompactOpacity = compactOpacity;
  const effectiveRowMorph = rowMorph;
  ...
  const navWidth = topWidth + (compactWidth - topWidth) * headerLayoutProgress;
  const navHeight = 118 - 68 * headerLayoutProgress;
  const headerPadTop = viewport > 940 ? 8 : 18 - 10 * headerLayoutProgress;
  const headerPadBottom = 8 * headerLayoutProgress;
  const headerBlur = 11 * headerVisualProgress;
  const groupNavY = -18 * headerVisualProgress;
  const compactNavY = (1 - effectiveCompactOpacity) * 12;
  const headerPreferenceFinalX = 96;
  ...
  const headerPreferenceY = innerPage ? 0 : 104 * (1 - preferenceLiftProgress);
  const brandR = Math.round(255 + (17 - 255) * headerVisualProgress);
  const brandG = Math.round(255 + (21 - 255) * headerVisualProgress);
  const brandB = Math.round(255 + (17 - 255) * headerVisualProgress);
  ...
  document.documentElement.style.setProperty("--group-nav-opacity", effectiveGroupOpacity.toFixed(4));
  document.documentElement.style.setProperty("--compact-nav-opacity", effectiveCompactOpacity.toFixed(4));
  ...
  document.documentElement.style.setProperty("--header-bg-alpha", (0.58 * headerVisualProgress).toFixed(4));
  document.documentElement.style.setProperty("--header-border-alpha", (0.09 * headerVisualProgress).toFixed(4));
  ...
  document.documentElement.style.setProperty("--header-control-bg-alpha", (0.14 + headerVisualProgress * 0.78).toFixed(4));
  document.documentElement.style.setProperty("--header-control-bg-hover-alpha", (0.24 + headerVisualProgress * 0.72).toFixed(4));
  document.documentElement.style.setProperty("--header-control-border-alpha", (0.18 + headerVisualProgress * 0.28).toFixed(4));
  document.documentElement.style.setProperty("--header-control-shadow-alpha", (0.16 * (1 - headerVisualProgress)).toFixed(4));
  document.documentElement.style.setProperty("--pre-row-progress", effectiveRowMorph.toFixed(4));
  document.documentElement.style.setProperty("--pre-group-gap", `${(12 - 6 * effectiveRowMorph).toFixed(1)}px`);
  document.documentElement.style.setProperty("--header-brand-color", `rgb(${brandR}, ${brandG}, ${brandB})`);
  document.documentElement.style.setProperty("--header-shadow-alpha", (0.36 * (1 - headerVisualProgress)).toFixed(4));
  if (header) {
    header.classList.toggle("scrolled", progress > 0.02 || innerPage);
    header.classList.toggle("compact-ready", compactHeader);
    updateDesktopHeaderMorph(progress, effectiveRowMorph);
    updateHeaderBookingMorph(headerVisualProgress);
...
const mobileRouteStackQuery = window.matchMedia("(max-width: 760px)");
const mobileRouteStackGroups = Array.from(
  document.querySelectorAll("body:not(.inner-page) #routes .route-grid, .tours-page .tours-card-grid")
);

if (mobileRouteStackGroups.length) {
  let mobileRouteStackFrame = 0;
  let mobileRouteStackLastTime = 0;
  const mobileStackLerp = 0.1;
  const mobileRouteStackStates = new WeakMap(
    mobileRouteStackGroups.map((group) => [group, { current: 0, initialized: false }])
  );
  const mobileStackMotion = [
    { start: 0, end: 0.22, startY: 92, finalY: 82, x: -8, rotate: -2.2, scale: 0.96 },
    { start: 0.08, end: 0.5, finalY: 110, x: 8, rotate: 1.5, scale: 0.98 },
    { start: 0.42, end: 0.84, finalY: 190, x: 0, rotate: -0.8, scale: 1 }
  ];

  const smoothMobileStackProgress = (value) => value * value * (3 - 2 * value);

  const updateMobileRouteStack = (timestamp = performance.now()) => {
    mobileRouteStackFrame = 0;
    const elapsed = mobileRouteStackLastTime ? Math.min(timestamp - mobileRouteStackLastTime, 64) : 16.667;
    const lerpAmount = 1 - Math.pow(1 - mobileStackLerp, elapsed / 16.667);
    let keepAnimating = false;
    mobileRouteStackLastTime = timestamp;

    mobileRouteStackGroups.forEach((group) => {
      const cards = Array.from(group.querySelectorAll(":scope > .route-stack-stage > .route-card"));
      const state = mobileRouteStackStates.get(group);

      if (!mobileRouteStackQuery.matches) {
        state.current = 0;
        state.initialized = false;
        group.style.removeProperty("--mobile-stack-scroll-progress");
        cards.forEach((card) => {
      ...
      const scrollRange = Math.max(group.offsetHeight - window.innerHeight, 1);
      const targetProgress = clamp01(-groupRect.top / scrollRange);

      if (!state.initialized || reducedMotionQuery.matches) {
        state.current = targetProgress;
        state.initialized = true;
      } else {
        state.current += (targetProgress - state.current) * lerpAmount;
        if (Math.abs(targetProgress - state.current) > 0.0001) {
          keepAnimating = true;
        } else {
          state.current = targetProgress;
        }
      ...
      const groupProgress = state.current;
      const enterY = window.innerHeight + 48;
      group.style.setProperty("--mobile-stack-scroll-progress", groupProgress.toFixed(4));

      cards.forEach((card, index) => {
        const motion = mobileStackMotion[index] || mobileStackMotion[mobileStackMotion.length - 1];
        const rawProgress = clamp01((groupProgress - motion.start) / Math.max(motion.end - motion.start, 0.001));
        const progress = smoothMobileStackProgress(rawProgress);
        const startY = Number.isFinite(motion.startY) ? motion.startY : enterY;
        const y = startY + (motion.finalY - startY) * progress;
        const x = motion.x * progress;
        const rotate = motion.rotate * progress;
        const scale = 1 + (motion.scale - 1) * progress;

        card.style.setProperty("--mobile-stack-x", `${x.toFixed(2)}px`);
        card.style.setProperty("--mobile-stack-y", `calc(${y.toFixed(2)}px + env(safe-area-inset-top, 0px))`);
        card.style.setProperty("--mobile-stack-rotate", `${rotate.toFixed(3)}deg`);
        card.style.setProperty("--mobile-stack-scale", scale.toFixed(4));
      });
    });

    if (keepAnimating) {
      mobileRouteStackFrame = window.requestAnimationFrame(updateMobileRouteStack);
    }
  };

  const requestMobileRouteStackUpdate = () => {
    if (mobileRouteStackFrame) return;
    mobileRouteStackFrame = window.requestAnimationFrame(updateMobileRouteStack);
  };

  window.addEventListener("scroll", requestMobileRouteStackUpdate, { passive: true });
  window.addEventListener("resize", requestMobileRouteStackUpdate);
  mobileRouteStackQuery.addEventListener?.("change", requestMobileRouteStackUpdate);
  updateMobileRouteStack(performance.now());
}
...
const fallbackRoutePriceLkr = 60000;
const currencyRates = {
...
const currencyPreferenceVersion = "usd-default-20260627";
...
const storedCurrencyPreference = getLocalPreference("ceylon-currency", defaultCurrency);
const hasCurrentCurrencyPreference = getLocalPreference(currencyPreferenceVersionKey, "") === currencyPreferenceVersion
  && getLocalPreference(currencyUserPreferenceKey, "") === "true"
  && Boolean(currencyRates[storedCurrencyPreference]);
let activeCurrency = hasCurrentCurrencyPreference ? storedCurrencyPreference : defaultCurrency;
if (!hasCurrentCurrencyPreference) {
...
function getRoutePriceLkr(converter) {
  const routePrice = Number(converter?.dataset.priceLkr);
  return Number.isFinite(routePrice) && routePrice > 0 ? routePrice : fallbackRoutePriceLkr;
}

function formatRoutePrice(converter, currency) {
  const config = currencyRates[currency] || currencyRates.LKR;
  const symbol = currencySymbols[currency] || currencySymbols.LKR;
  const convertedPrice = getRoutePriceLkr(converter) * config.rate;
...
function updateRoutePrices(currency, animate = true) {
  priceConverters.forEach((converter) => {
    ...
    const nextText = formatRoutePrice(converter, currency);
...
function bindFaqMenus() {
  ...
  document.querySelectorAll("[data-faq-menu]").forEach((control) => {
    ...
    const fineFaqPointerQuery = window.matchMedia("(pointer: fine)");
    const scrollCursor = document.querySelector(".faq-scroll-cursor");
    ...
    function hideFaqScrollCursor() {
      document.documentElement.classList.remove("faq-scroll-cursor-active");
    }

    function updateFaqScrollCursor(event) {
      if (!scrollCursor || mobileFaqMenuQuery.matches || !fineFaqPointerQuery.matches || !isFaqWheelHit(event)) {
        hideFaqScrollCursor();
        return;
      }

      document.documentElement.style.setProperty("--faq-scroll-cursor-x", `${event.clientX}px`);
      document.documentElement.style.setProperty("--faq-scroll-cursor-y", `${event.clientY}px`);
      document.documentElement.classList.add("faq-scroll-cursor-active");
    }

    function handleFaqWheel(event) {
      if (mobileFaqMenuQuery.matches) return;
      if (Math.abs(event.deltaY) < 2) return;
      if (!isFaqWheelHit(event)) {
        hideFaqScrollCursor();
        return;
      }
      updateFaqScrollCursor(event);
      event.preventDefault();
    ...
    wheelZone.addEventListener("pointermove", updateFaqScrollCursor, { passive: true });
    wheelZone.addEventListener("pointerleave", hideFaqScrollCursor);
    mobileFaqMenuQuery.addEventListener("change", hideFaqScrollCursor);
    fineFaqPointerQuery.addEventListener("change", hideFaqScrollCursor);
    window.addEventListener("blur", hideFaqScrollCursor);
...
if (blogProjectsStage && blogProjectCards.length) {
  ...
  const updateBlogProjectsStage = () => {
    ...
    const animationDistance = Math.max(scrollableDistance * 0.92, viewportHeight * 1.55, 1);
...
const stayGalleryStates = new WeakMap();

document.querySelectorAll("[data-stay-gallery]").forEach((gallery) => {
  const image = gallery.querySelector("[data-stay-gallery-image]");
  const sourcesScript = gallery.querySelector("[data-stay-gallery-sources]");
  const prevButton = gallery.querySelector("[data-stay-gallery-prev]");
  const nextButton = gallery.querySelector("[data-stay-gallery-next]");
  const dotsWrap = gallery.querySelector("[data-stay-gallery-dots]");
  if (!image || !sourcesScript || !prevButton || !nextButton || !dotsWrap) return;

  let sources = [];
  try {
    sources = JSON.parse(sourcesScript.textContent || "[]");
  } catch (error) {
    sources = [];
  }
  ...
  const galleryName = gallery.dataset.stayGalleryName
    || gallery.querySelector(".itinerary-stay-gallery-header strong")?.textContent?.trim()
    || "stay";
  let currentIndex = 0;
  const loadedSources = new Set([sources[0].src]);
  const dots = sources.map((source, index) => {
    const dot = document.createElement("button");
    dot.className = "itinerary-stay-gallery-dot";
    dot.type = "button";
    dot.dataset.stayGalleryDotIndex = String(index);
    dot.setAttribute("aria-label", `Show ${galleryName} photo ${index + 1}`);
    dotsWrap.appendChild(dot);
    return dot;
  });

  const preloadStayGalleryImage = (index) => {
    const source = sources[(index + sources.length) % sources.length];
    if (!source || loadedSources.has(source.src)) return;
    loadedSources.add(source.src);
    const preloadImage = new Image();
    preloadImage.decoding = "async";
    preloadImage.src = source.src;
  };

  function showStayGalleryImage(index) {
    ...
    window.setTimeout(() => {
      ...
      image.alt = source.alt || `${galleryName} hotel photo ${currentIndex + 1} of ${sources.length}`;
    ...
    preloadStayGalleryImage(currentIndex + 1);
    preloadStayGalleryImage(currentIndex - 1);
  }

  stayGalleryStates.set(gallery, {
    getCurrentIndex: () => currentIndex,
    show: showStayGalleryImage
  });
  showStayGalleryImage(0);
});

document.addEventListener("click", (event) => {
  const target = event.target;
  const control = target?.closest?.("[data-stay-gallery-prev], [data-stay-gallery-next], [data-stay-gallery-dot-index]");
  if (!control) return;

  const gallery = control.closest("[data-stay-gallery]");
  const state = gallery ? stayGalleryStates.get(gallery) : null;
  if (!state) return;

  event.preventDefault();
  if (control.hasAttribute("data-stay-gallery-prev")) {
    state.show(state.getCurrentIndex() - 1);
    return;
  }
  if (control.hasAttribute("data-stay-gallery-next")) {
    state.show(state.getCurrentIndex() + 1);
    return;
  }

  const dotIndex = Number(control.dataset.stayGalleryDotIndex);
  if (Number.isFinite(dotIndex)) state.show(dotIndex);
});
...
countEls.forEach((el) => {
  ...
  el.textContent = `${el.dataset.start}${suffix}`;
});
...
const bookingTranslations = {
  en: {
    ...
    empty: "N/A",
    requestQuotation: "Request Detailed Quotation",
...
function initBookingPassengerMenu(form, peopleInput) {
  ...
  const closeOpenBookingCalendars = () => {
    document.querySelectorAll("[data-booking-calendar].is-open").forEach((openCalendar) => {
      openCalendar.classList.remove("is-open");
      openCalendar.closest(".booking-step")?.classList.remove("has-open-calendar");
      openCalendar.querySelector("[data-booking-date-display]")?.setAttribute("aria-expanded", "false");
    });
  };
  ...
  peopleInput.addEventListener("focus", () => {
    if (window.matchMedia("(max-width: 760px)").matches) closeOpenBookingCalendars();
  });
  peopleInput.addEventListener("pointerdown", () => {
    if (window.matchMedia("(max-width: 760px)").matches) closeOpenBookingCalendars();
  }, { passive: true });
...
function initBookingCalendar(form) {
  ...
  const getFixedRouteDays = () => {
    ...
    const routeSlug = getCanonicalBookingRouteSlug(selectedRoute?.dataset.routeSlug || selectedRoute?.value || "");
    if (!selectedRoute || routeSlug === "custom-route") return null;
    const routeDays = Number(selectedRoute.dataset.days);
    return Number.isFinite(routeDays) && routeDays > 0 ? routeDays : null;
  };
...
if (bookingForm) {
  const BOOKING_VEHICLE_MAINTENANCE_LKR_PER_DAY = 40000;
  const BOOKING_VEHICLE_FUEL_LKR_PER_ROUTE = 40000;
  const BOOKING_DRIVER_LKR_PER_DAY = 5000;
  ...
  const getBookingRouteDays = (selectedRoute) => {
    const routeDays = Number(selectedRoute?.dataset.days);
    return Number.isFinite(routeDays) && routeDays > 0 ? routeDays : 1;
  };

  const getBookingVehicleCostLkr = (selectedRoute) => {
    const days = getBookingRouteDays(selectedRoute);
    return (BOOKING_VEHICLE_MAINTENANCE_LKR_PER_DAY + BOOKING_DRIVER_LKR_PER_DAY) * days
      + BOOKING_VEHICLE_FUEL_LKR_PER_ROUTE;
  };

  const getBookingPassengerCostLkr = (selectedRoute, adultCount, childCount) => {
    const foodCost = Number(selectedRoute?.dataset.food || 0);
    const accommodationCost = Number(selectedRoute?.dataset.accommodation || 0);
    const adultRouteCost = (Number.isFinite(foodCost) ? foodCost : 0)
      + (Number.isFinite(accommodationCost) ? accommodationCost : 0);
    return adultRouteCost * adultCount + (adultRouteCost / 2) * childCount;
  };
...
if (contactForm) {
  document.querySelectorAll("[data-contact-topic-menu]").forEach((control) => {
    ...
    const placeholderText = activeLabel.dataset.contactTopicPlaceholder || activeLabel.textContent.trim();

    function showContactTopicPlaceholder() {
      activeLabel.textContent = placeholderText;
      activeLabel.classList.add("is-placeholder");
      hiddenInput.value = "";
    ...
    function setContactTopic(option, close = false) {
      ...
      activeLabel.classList.remove("is-placeholder");
    ...
    if (hiddenInput.value && selectedOption) setContactTopic(selectedOption);
    else showContactTopicPlaceholder();
  });
  ...
  const resetContactTopicMenus = () => {
    document.querySelectorAll("[data-contact-topic-menu]").forEach((control) => {
      ...
      const placeholderText = activeLabel.dataset.contactTopicPlaceholder || "How can we help?";

      const nextValue = hiddenInput.value || options[0].dataset.topicValue || options[0].textContent.trim();
      const selectedOption = options.find((option) => (option.dataset.topicValue || option.textContent.trim()) === nextValue);

      if (selectedOption && hiddenInput.value) {
...
const termsIndexLinks = Array.from(document.querySelectorAll(".terms-index a[href^='#']"));
if (termsIndexLinks.length) {
  const termsIndex = document.querySelector(".terms-index");
  const termsIndexList = termsIndex?.querySelector("ol");
  const termsSections = termsIndexLinks
    .map((link) => document.querySelector(link.getAttribute("href")))
    .filter(Boolean);

  const setActiveTermsSection = (sectionId) => {
    let activeIndex = 0;
    termsIndexLinks.forEach((link) => {
      const isActive = link.getAttribute("href") === `#${sectionId}`;
      if (isActive) activeIndex = termsIndexLinks.indexOf(link);
      link.classList.toggle("is-active", isActive);
      link.closest("li")?.classList.toggle("is-active", isActive);
    });

    if (termsIndex && termsIndexList) {
      const shouldShiftIndex = activeIndex >= 15;
      const maxShift = Math.max(0, termsIndexList.scrollHeight - termsIndex.clientHeight);
      const activeLink = termsIndexLinks[activeIndex];
      const targetShift = shouldShiftIndex && activeLink
        ? Math.min(maxShift, Math.max(0, activeLink.offsetTop - termsIndex.clientHeight * 0.58))
        : 0;
      termsIndex.style.setProperty("--terms-index-shift", `${Math.round(targetShift)}px`);
    }
  };

  const updateActiveTermsSection = () => {
    const anchorLine = window.innerHeight * 0.36;
    let activeSection = termsSections[0];
    ...
    if (activeSection) setActiveTermsSection(activeSection.id);
  };

  let termsScrollTicking = false;
  const requestTermsUpdate = () => {
    if (termsScrollTicking) return;
    termsScrollTicking = true;
    window.requestAnimationFrame(() => {
      updateActiveTermsSection();
      termsScrollTicking = false;
    });
  };
  ...
  termsIndexLinks.forEach((link) => {
    link.addEventListener("click", () => {
      ...
      setActiveTermsSection(sectionId);
    });
  });
...
if (blogProfileReveals.length) {
  const blogProfileCredits = Array.from(document.querySelectorAll(".blog-article-credit"));
  const mobileProfileRevealQuery = window.matchMedia("(max-width: 760px)");
  let blogProfileRevealTimers = [];

  const clearBlogProfileRevealTimers = () => {
    blogProfileRevealTimers.forEach((timer) => window.clearTimeout(timer));
    blogProfileRevealTimers = [];
  };

  const runMobileBlogProfileReveal = () => {
    clearBlogProfileRevealTimers();
    blogProfileReveals.forEach((profile) => profile.classList.remove("is-mobile-auto"));
    blogProfileCredits.forEach((credit) => credit.classList.remove("is-mobile-auto"));

    if (!mobileProfileRevealQuery.matches) return;

    blogProfileRevealTimers.push(window.setTimeout(() => {
      blogProfileReveals.forEach((profile) => profile.classList.add("is-mobile-auto"));
      blogProfileCredits.forEach((credit) => credit.classList.add("is-mobile-auto"));
    }, 420));
  ...
  runMobileBlogProfileReveal();
  mobileProfileRevealQuery.addEventListener("change", runMobileBlogProfileReveal);
}

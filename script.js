...
function updateMobileHeaderVisibility(y = window.scrollY || 0) {
  ...
  if (isHomeGalleryPhotoStageActive()) {
    const headerHeight = Math.ceil(header.offsetHeight || header.getBoundingClientRect().height || 64);
...
function getCriticalLoaderImagePromises() {
  ...
  if (body.classList.contains("about-page")) {
    promises.push(waitForCriticalImageUrl("assets/optimized/about-hero-hulu-fast.jpg"));
...
function finishLoaderAfterCriticalImages() {
  const criticalImagePromises = getCriticalLoaderImagePromises();

  if (criticalImagePromises.length) {
    document.documentElement.classList.remove("skip-loader");
  }

  Promise.all(criticalImagePromises).then(finishLoader);
}

const shouldBypassLoader = shouldSkipLoader && getCriticalLoaderImagePromises().length === 0;
